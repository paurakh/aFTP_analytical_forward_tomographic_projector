function [ im ] = ifanBeam4AnalyticalGPU01(sinogram, SDD, SID, fOV, fOVR, xSize, ySize, nDet, nViews, detectorOffset, rotationRange )
%% back projection
...
sinogram1 = sinogram*SID.* repmat(cos(relFanAngle*pi/180)',[1 nViews]);
% sinogram1 = sinogram;%*SID.*repmat(cos(relFanAngle*pi/180)',[1 nViews]);
...
parfor iView = 1:nViews
    ...
    %     testq(:,iView) = q';
    q = conv(squeeze(sinogram1(:,iView)), kernel,'same')';
%          q = sinogram1(:,iView)';
...
%     absAngle = gather(absAngle);
    ...
    %     val = (1 - (absAngle -angle1)./(angle2 - angle1)) .* val1; ...
    %         + (absAngle -angle1)./(angle2 - angle1) .* val2 ;
    %     val = val1 + (absAngle -angle1)./(angle2 - angle1).*((val2-val1));
    val = (1-term1) .* val1 + term1 .* val2;
    %           val = val2;
    %        val = val1;
...
im = (flipud(im*1.6*pi/(nDet*nViews)));
