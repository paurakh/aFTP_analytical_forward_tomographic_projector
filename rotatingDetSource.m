...
if not(exist('a'))
    ...
    a = 30; % [mm]
    b = 10; % [mm]
    % center coordinate (h, k)
    h = 20; % [mm]
    k = 20; % [mm]
    % angle of major semi axis to the x-axis
    alpha = pi/4; %[radians]
end
%%
Ns = 300;        % number of detectors
NTheta = 360;    % number of views
mmPerSample = 1; % 1 mmPerSample of detector
...
theta = linspace(1e-5,2*pi-1*pi/180+1e-5, NTheta);  % rotation angle array
