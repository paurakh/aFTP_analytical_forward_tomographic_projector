function [img,H] = iradonVarT(varargin)
...
% Design the filter
len = size(p,1);
[H hImp] = designFilter(filter, len, d);
...
kernel = ifftshift(ifft((H)));
for i = 1:size(p,2)
    pSlice = p(:,i);
    p(:,i) = conv(squeeze(pSlice), kernel.^2 , 'same')';
end
...
%======================================================================
function [filt kernel] = designFilter(filter, len, d)
...
filtSq = 2^2*real(fft(filtImpResp.^2));
filtSq = filtSq(1:(order/2)+1);
...
kernel = ifft(ifftshift(filt));


filtSq(w>pi*d) = 0;                      % Crop the frequency response
filtSq = [filtSq' ; filtSq(end-1:-1:2)'];    % Symmetry of the filtSqer
%----------------------------------------------------------------------
