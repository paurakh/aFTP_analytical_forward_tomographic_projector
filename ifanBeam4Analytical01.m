function [ im ] = ifanBeam4Analytical01(sinogram, SDD, SID, fOV, fOVR, xSize, ySize, nDet, nViews, detectorOffset, rotationRange )
%% back projection
...
%% Attempt at sinc interpolation    
%     r1 = max(1, i1-2);
%     r2 = min(i2+2,length(q));
%     for i = 1: length(r1)
%         x = q(r1(i):r2(i))';
%         t = absFanAngle(r1(i):r2(i));
%         ts = absAngle(i);
%         [Ts,T] = ndgrid(ts,t);
%         val(i) = (sinc(Ts - T)*x)';
%     end
%     x = q(r1:r2)';
%     t = absFanAngle(max(1, i1-2):min(i2+2,length(q)));
%     
%     ts = absAngle;
%     [Ts,T] = ndgrid(ts,t);
%     val = (sinc(Ts - T)*x)';
